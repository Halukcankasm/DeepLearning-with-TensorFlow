...
y1 = tf.nn.relu(tf.nn.conv2d(x, weight_1, strides=[1,1,1,1], padding="SAME"))#output=[28,28,16]
y1=tf.nn.max_pool(y1,ksize=[1,2,2,1],strides=[1,2,2,1],padding="SAME")#output=[14,14,16]

y2=tf.nn.relu(tf.nn.conv2d(y1, weight_2, strides=[1,1,1,1], padding="SAME"))#output=[14,14,32]
...
optimize = tf.train.AdamOptimizer(5e-4).minimize(loss)  #5e-4 = 0.0005
