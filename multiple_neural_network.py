...
layer_1=128 #How many neurons
layer_2=64
layer_3=32
layer_out=10

weight_1 = tf.Variable(tf.truncated_normal([784,layer_1], stddev=0.1))
bias_1 = tf.Variable(tf.constant(0.1,shape=[layer_1]))#number of neuron

weight_2 = tf.Variable(tf.truncated_normal([layer_1,layer_2], stddev=0.1))
bias_2 = tf.Variable(tf.constant(0.1,shape=[layer_2]))#number of neuron

weight_3 = tf.Variable(tf.truncated_normal([layer_2,layer_3], stddev=0.1))
bias_3 = tf.Variable(tf.constant(0.1,shape=[layer_3]))#number of neuron

weight_4 = tf.Variable(tf.truncated_normal([layer_3,layer_out], stddev=0.1))
bias_4 = tf.Variable(tf.constant(0.1,shape=[layer_out]))#number of neuron

y1=tf.nn.relu(tf.matmul(x,weight_1)+bias_1) # for hidden layer 1
y1d=tf.nn.dropout(y1,pkeep)
y2=tf.nn.relu(tf.matmul(y1d,weight_2)+bias_2) # for hidden layer 2
y2d=tf.nn.dropout(y2,pkeep)
y3=tf.nn.relu(tf.matmul(y2d,weight_3)+bias_3) # for hidden layer 3
...
def training_step(iterations):
    for i in range(iterations):
        ...
        loss_graph_list.append(train_loss)
        
        if i % 100 == 0:
            train_acc = sess.run(accuracy,feed_dict=feed_dict_train)
            print("Iterations:",i,"Training acc:",train_acc,"Trainning loss:",train_loss)
